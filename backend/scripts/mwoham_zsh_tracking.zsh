...
if [[ -n "${MWOHAM_ZSH_TRACKING_LOADED:-}" ]]; then
  return 0
fi
...
_mwoham_should_skip_command() {
  ...
  case "$command_text" in
    cd|cd\ *|pwd|pwd\ *|ls|ls\ *|clear|clear\ *) return 0 ;;
...
_mwoham_command_tracking_precmd() {
  ...
  local duration_ms="0"
  if [[ -n "$MWOHAM_CMD_TRACKING_STARTED_EPOCH" && -n "$ended_epoch" ]]; then
    duration_ms=$(printf "%.0f" "$(( (ended_epoch - MWOHAM_CMD_TRACKING_STARTED_EPOCH) * 1000 ))" 2>/dev/null)
  fi
  ...
  (
    ...
    uv run python scripts/record_command_result.py \
      ...
      --cwd "$MWOHAM_CMD_TRACKING_CWD" \
      --started-at "$MWOHAM_CMD_TRACKING_STARTED_AT" \
      --ended-at "$ended_at" \
      --shell "zsh" \
      --source "terminal" \

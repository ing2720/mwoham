...
이 문서는 v0.4.0 meeting transcription 이후 시스템 오디오 캡처와 Apple Speech 연결 가능성을 확인한 기술 검증 기록입니다.

이번 spike는 시스템 오디오를 실제 회의 전사 입력으로 쓸 수 있는지 확인하기 위한 작업입니다. Whisper/OpenAI STT, 화자 분리, 고급 회의 요약은 포함하지 않습니다.
